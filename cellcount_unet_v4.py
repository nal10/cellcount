...
# Architecture simplified to have less parameters
...
validation_generator = DataGeneratorVal(testing_partition, batch_size)

input_im = Input(shape=(64, 64, 1), name='input_im')
conv2 = Conv2D(8, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(input_im)    # (batch, 64, 64, 8)
conv2 = Conv2D(8, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv2)	    # (batch, 64, 64, 8)
pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)			    # (batch, 32, 32, 8)
conv3 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(pool2)		# (batch, 32, 32, 4)
conv3 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv3)		# (batch, 32, 32, 4)
pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)				# (batch, 16, 16, 4)
conv4 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(pool3)		# (batch, 16, 16, 4)
conv4 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv4)		# (batch, 16, 16, 4)
drop4 = Dropout(0.5)(conv4)									# (batch, 16, 16, 4)
pool4 = MaxPooling2D(pool_size=(2, 2))(drop4)				# (batch, 8, 8, 4)
conv5 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(pool4)		# (batch, 8, 8, 4)
conv5 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv5)		# (batch, 8, 8, 4)
drop5 = Dropout(0.5)(conv5)									# (batch, 8, 8, 4)
up6 = UpSampling2D(size=(2, 2))(drop5)						# (batch, 16, 16, 4)
up6 = Conv2D(4, 2, activation='relu', padding='same',
             kernel_initializer='he_normal')(up6)			# (batch, 16, 16, 4)
cat6 = Concatenate(axis=3)([drop4, up6])  					# (batch, 16, 16, 4)
conv6 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(cat6)		# (batch, 16, 16, 4)
conv6 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv6)		# (batch, 16, 16, 4)
up7 = UpSampling2D(size=(2, 2))(conv6)						# (batch, 32, 32, 4)
up7 = Conv2D(4, 2, activation='relu', padding='same',
             kernel_initializer='he_normal')(up7)			# (batch, 32, 32, 4)
cat7 = Concatenate(axis=3)([conv3, up7])				    # (batch, 32, 32, 4)
conv7 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(cat7)		# (batch, 32, 32, 4)
conv7 = Conv2D(4, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv7)		# (batch, 32, 32, 4)
up8 = UpSampling2D(size=(2, 2))(conv7)						# (batch, 64, 64, 4)
up8 = Conv2D(4, 2, activation='relu', padding='same',
             kernel_initializer='he_normal')(up8)			# (batch, 64, 64, 4)
cat8 = Concatenate(axis=3)([conv2, up8])  					# (batch, 64, 64, 256)
conv8 = Conv2D(8, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(cat8)		# (batch, 64, 64, 8)
conv8 = Conv2D(8, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv8)		# (batch, 64, 64, 8)
conv9 = Conv2D(2, 3, activation='relu', padding='same',
               kernel_initializer='he_normal')(conv8)		# (batch, 64, 64, 2)
output_im = Conv2D(1, 1, activation='sigmoid',
                   name='output_im')(conv9)		            # (batch, 64, 64, 1)
...
if runmode == 'trainnew':
    ...
    model.load_weights(base_path + rel_result_path + '/' + 'v4Labels_64smallArch_wBCE_continue_v4_continued' + '/' + '0050' + '.h5')
    train_history = model.fit_generator(training_generator, epochs=nepochs, verbose=1, callbacks=[hist_cb], validation_data=validation_generator,
                                        max_queue_size=10, workers=1, use_multiprocessing=True, shuffle=True, initial_epoch=0)
    summary = train_history.params
    summary.update(train_history.history)
