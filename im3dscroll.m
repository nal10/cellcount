function [] = im3dscroll(IM,IMlbl,r,tt)
...
hf.UserData.IM=IM;
hf.UserData.IMlbl=IMlbl;
hf.UserData.currplane=1;
hf.UserData.h_im=h_im;
hf.UserData.tt=tt;
ha.Title.String=['Current plane: ',num2str(hf.UserData.tt(hf.UserData.currplane))];
...
sld = uicontrol('Style', 'slider',...
    'Min',1,'Max',size(IM,3),'SliderStep',[1,1]./(size(IM,3)-1),'Value',1,...
    'Position', [400 20 120 20],...
    'Callback', @change_z, 'Parent', hf);

set(hf,'KeyPressFcn',@Key_Down);
end


function Key_Down(src,ed)
hf=gcbf;
ha=findobj(hf.Children,'flat','Type','Axes');
hi=hf.UserData.h_im;
if strcmp(ed.Key,'rightarrow')
    hi.CData=hf.UserData.IM(:,:,hf.UserData.currplane);
elseif strcmp(ed.Key,'leftarrow')
    hi.CData=hf.UserData.IMlbl(:,:,hf.UserData.currplane);
end
end

function change_z(src,ed)
hf=gcbf;
ha=findobj(hf.Children,'flat','Type','Axes');
hi=hf.UserData.h_im;

%disp(src.Value)
hf.UserData.currplane=src.Value;
hi.CData=hf.UserData.IM(:,:,hf.UserData.currplane);
ha.Title.String=['Current plane: ',num2str(hf.UserData.tt(hf.UserData.currplane))];
end
