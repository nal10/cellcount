...
output_im_1 = Conv2D(1, 1, activation='sigmoid', name='output_im_1')(conv9)
output_im_2 = Conv2D(1, 1, activation='sigmoid', name='output_im_2')(conv9)

model = Model(inputs=[input_im], outputs=[output_im_1,output_im_2])
...
model.compile(optimizer=Adam(),
              loss={'output_im_1': loss_fcn_wbce,
                    'output_im_2': loss_fcn_wbce},
              metrics=['accuracy'])
...
if runmode == 'train-new':
    ...
    train_history = model.fit_generator(train_generator,
                                        validation_data=[],
                                        initial_epoch=100, epochs=nepochs,
                                        max_queue_size=20, workers=10,
                                        use_multiprocessing=True, shuffle=True,
                                        verbose=2, callbacks=[hist_cb],
                                        )
    ...
    with open(base_path + rel_result_path + runid+'100-200-summary'+'.pkl', 'wb') as file_pi:
        cPickle.dump(summary, file_pi)
...
val_lbl_fg = combine_patches(val_lbl_pred[0],patchsize=D.patchsize,stride=D.stride,imsize=D.im_buffer[0].shape,padding=True)
val_lbl_bo = combine_patches(val_lbl_pred[1],patchsize=D.patchsize,stride=D.stride,imsize=D.im_buffer[0].shape,padding=True)

plt.ion()
#plt.figure()
#ax1 = plt.imshow(lbl_pred_full)
